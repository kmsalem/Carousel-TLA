...
P0_(self) == /\ pc[self] = "P0_"
             /\ IF counter[self] < NumOfMessages
                   THEN /\ counter' = [counter EXCEPT ![self] = counter[self] + 1]
                        /\ channels' = [channels EXCEPT ![self] = Append((channels[self]), counter'[self])]
...
StatusInvariant == \A x \in 1..N:
                status[x] = "Committed" \/ status[x] = "Initiateda"
