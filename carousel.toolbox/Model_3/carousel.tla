...
\*  Deadlock here
    while copy # {} do
        with selectedServer \in copy do
\*          Remove selected from serverSet
            print selectedServer;
            copy := copy \ {selectedServer};
...
\* Label P0 of procedure sendClientMessagesToServers at line 111 col 5 changed to P0_
\* Process variable msg of process sendClientMessages at line 123 col 5 changed to msg_
...
VARIABLES IDSet, status, sent, received, channels, pc, stack, msg_c, client_, 
          tmp, msg, id, serverStatus, client, serverSet, msg_s, copy, msg_, 
          head, subsets
...
Init == (* Global variables *)
        ...
        /\ copy = [ self \in ProcSet |-> serverSet[self]]
...
P1(self) == /\ pc[self] = "P1"
            ...
            /\ UNCHANGED << status, sent, received, channels, stack, msg_c, 
                            ...
                            msg_s, copy, msg_, head, subsets >>

createServerMsg(self) == P1(self)

P0_(self) == /\ pc[self] = "P0_"
             /\ IF copy[self] # {}
                   THEN /\ \E selectedServer \in copy[self]:
                             /\ PrintT(selectedServer)
                             /\ copy' = [copy EXCEPT ![self] = copy[self] \ {selectedServer}]
...
P0(self) == /\ pc[self] = "P0"
            /\ \E chosen \in subsets[self]:
                 ...
                 /\ copy' = [copy EXCEPT ![self] = serverSet'[self]]
                 /\ pc' = [pc EXCEPT ![self] = "P0_"]
            /\ UNCHANGED << IDSet, status, sent, received, channels, msg_c, 
                            client_, tmp, msg, id, serverStatus, msg_, head, 
                            subsets >>
